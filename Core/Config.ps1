<#
.SYNOPSIS
    Configuration management for NoID Privacy Pro Framework
    ...
    $defaultConfig = @{
        ...
        modules = @{
            ...
            AdvancedSecurity = @{
                ...
                profiles = @("Home", "EnterpriseConservative", "AirGappedMax")
            }
        }
        options = @{
            dryRun = $false
    ...
    foreach ($prop in $script:Config.modules.PSObject.Properties) {
        ...
        if ($moduleName -eq "DNS") {
            ...
            if (Get-Member -InputObject $moduleConfig -Name 'provider' -MemberType NoteProperty) {
                $validProviders = @('Cloudflare', 'Quad9', 'AdGuard', '')

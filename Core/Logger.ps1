<#
.SYNOPSIS
    Unified logging system for NoID Privacy Pro Framework
...
# Global logger configuration - Initialize if not already set
if (-not (Get-Variable -Name 'LoggerConfig' -Scope Script -ErrorAction SilentlyContinue)) {
    $script:LoggerConfig = @{
        LogFilePath = ""
...
function Initialize-Logger {
    ...
    $script:LoggerConfig.LogFilePath = Join-Path $LogDirectory $logFileName
    $script:LoggerConfig.MinimumLevel = $MinimumLevel
    $script:LoggerConfig.EnableConsole = $EnableConsole
    $script:LoggerConfig.EnableFile = $EnableFile
    
    # Test if we can write to the log file
    if ($EnableFile) {
        try {
            "# NoID Privacy Pro Log File" | Out-File -FilePath $script:LoggerConfig.LogFilePath -Encoding UTF8 -ErrorAction Stop
        }
        catch {
            Write-Host "[ERROR] Failed to create log file: $($script:LoggerConfig.LogFilePath)" -ForegroundColor Red
            Write-Host "[ERROR] Exception: $_" -ForegroundColor Red
            $script:LoggerConfig.EnableFile = $false
        }
    }
    
    # Write initial log entry
    Write-Log -Level INFO -Message "Logger initialized" -Module "Logger"
    Write-Log -Level INFO -Message "Log file: $($script:LoggerConfig.LogFilePath)" -Module "Logger"
}
...
function Write-Log {
    ...
    if ($Level -lt $script:LoggerConfig.MinimumLevel) {
        return
    }
    ...
    if ($script:LoggerConfig.EnableFile -and $script:LoggerConfig.LogFilePath) {
        $maxRetries = 5
        $retryDelayMs = 100
        $writeSuccess = $false
        $lastError = $null
        ...
        for ($i = 0; $i -lt $maxRetries; $i++) {
            try {
                Add-Content -Path $script:LoggerConfig.LogFilePath -Value $logEntry -Encoding UTF8 -ErrorAction Stop
                $writeSuccess = $true
                break
            }
            catch {
        ...
        if (-not $writeSuccess) {
            ...
            Write-Host "[FILE WRITE ERROR] Failed to write to log file after $maxRetries attempts: $($script:LoggerConfig.LogFilePath)" -ForegroundColor Red
    ...
    if ($script:LoggerConfig.EnableConsole -and $Level -ge [LogLevel]::INFO) {
...
function Get-LogFilePath {
    ...
    #>
    return $script:LoggerConfig.LogFilePath
}

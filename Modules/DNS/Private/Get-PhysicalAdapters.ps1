function Get-PhysicalAdapters {
    ...
    param(
        ...
        [Parameter()]
        [switch]$RequireVpnInspection
    )

    # This helper is also loaded by the action-scoped DNS Quick Action, where
    # the DNS module's script scope intentionally does not exist. Keep logging
    # local so adapter selection has no hidden dependency on Import-Module.
    $logModule = 'DNS'
    $moduleNameVariable = Get-Variable -Name ModuleName -Scope Script -ErrorAction SilentlyContinue
    if ($moduleNameVariable -and
        -not [string]::IsNullOrWhiteSpace([string]$moduleNameVariable.Value)) {
        $logModule = [string]$moduleNameVariable.Value
    }

    try {
        Write-Log -Level DEBUG -Message "Retrieving physical network adapters..." -Module $logModule
        ...
        if (-not $IncludeDisabled) {
            ...
            Write-Log -Level DEBUG -Message "Filtering to active/disconnected adapters (excluding disabled)" -Module $logModule
        }

        # Layer 3: Exclude known host-side virtual/VPN adapter patterns.
        ...
        try {
            $vpnConns = @(Get-VpnConnection -AllUserConnection -ErrorAction Stop) +
                @(Get-VpnConnection -ErrorAction Stop)
            $activeVpnConnections = @($vpnConns | Where-Object { $_.ConnectionStatus -eq 'Connected' })
        }
        catch {
            if ($RequireVpnInspection) {
                # Adapter exclusion is part of a mutation/backup scope. If
                # native VPN state cannot be inspected, do not silently widen
                # that scope.
                throw "Native VPN connection lookup was unavailable: $($_.Exception.Message)"
            }
            # Read-only status and compliance calls still audit physical links
            # when RasMan/native VPN inventory is unavailable. Make the missing
            # exclusion evidence visible without turning an unrelated probe
            # into a total loss of audit availability.
            Write-Log -Level WARNING -Message "Native VPN connection lookup was unavailable during read-only adapter inspection; physical adapters are reported without native-VPN exclusion: $($_.Exception.Message)" -Module $logModule
        }
        ...
        $filteredAdapters = @($adapters | Where-Object {
            ...
            if ($skipAdapter) {
                Write-Log -Level DEBUG -Message "Excluding adapter: $name - $skipReason" -Module $logModule
            }
        ...
        if ($filteredAdapters.Count -eq 0) {
            Write-Log -Level WARNING -Message "No physical network adapters found" -Module $logModule
            return @()
        }

        Write-Log -Level DEBUG -Message "Found $($filteredAdapters.Count) physical network adapter(s)" -Module $logModule

        foreach ($adapter in $filteredAdapters) {
            Write-Log -Level DEBUG -Message "  - $($adapter.Name) ($($adapter.InterfaceDescription)) [Status: $($adapter.Status)]" -Module $logModule
        }

        return $filteredAdapters  # Already wrapped as array in line 83
    }
    catch {
        Write-ErrorLog -Message "Failed to retrieve physical network adapters" -Module $logModule -ErrorRecord $_
        throw
    }
}

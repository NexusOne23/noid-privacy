    ...
    try {
        ...
        if ($newValue -eq 0) {
            ...
            elseif ($null -eq $currentValue) {
                Write-Log -Level INFO -Message "WDigest setting was not configured (default varies by OS version)" -Module "AdvancedSecurity"
            }
            else {
                Write-Log -Level INFO -Message "WDigest credential protection was already enabled (no change needed)" -Module "AdvancedSecurity"
            }

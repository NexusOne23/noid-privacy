    ...
    process {
        try {
            ...
            if (-not $defenderService -or $defenderService.Status -ne "Running") {
                throw "Windows Defender service is not running. ASR rules require Defender to be active."
            }
            ...
            # ALWAYS ask user about management tools (not just when detected!)
            $usesManagementTools = $false
            
            if (-not $Force -and -not $AllowPSExecWMI -and -not $DryRun) {
                Write-Host ""
            ...
            # Step 2b: Interactive choice for prevalence rule (new/unknown software)
            if (-not $DryRun) {

...
function Invoke-ASRRules {
    ...
    process {
        try {
            ...
            # - In both cases, ASR rules are not enforceable by NoID (ASR is a Defender-only API)
            ...
            if ($securityProduct.Detected) {
                ...
                Write-Host "========================================" -ForegroundColor Yellow
                Write-Host ""
                Write-Host "Third-party endpoint product detected: $avName" -ForegroundColor Cyan
                Write-Host ""
                Write-Host "ASR (Attack Surface Reduction) is a Microsoft-Defender-specific API and" -ForegroundColor Yellow
                Write-Host "cannot be configured by NoID when Defender is not the primary engine." -ForegroundColor Yellow
                Write-Host ""
                Write-Host "Your product ($avName) likely provides its own attack-surface-reduction" -ForegroundColor Gray
                Write-Host "features (behavioral analysis, exploit prevention, ML-based detection," -ForegroundColor Gray
                Write-Host "etc.). NoID cannot verify those -- please consult your vendor's documentation" -ForegroundColor Gray
                Write-Host "or management console to confirm equivalent protections are enabled." -ForegroundColor Gray
                Write-Host ""
                Write-Host "This is NOT an error - ASR will be skipped." -ForegroundColor Green
                Write-Host ""

                Write-Log -Level WARNING -Message "ASR skipped: Third-party endpoint product detected ($avName). Detection: $($securityProduct.DetectionMethod). NoID cannot verify whether the product has equivalent ASR features -- consult vendor documentation." -Module $moduleName

                $result.Success = $true  # Not an error - intentional skip
                $result.Warnings += "ASR skipped: Third-party endpoint product detected ($avName). NoID cannot verify whether the product provides equivalent attack-surface-reduction features -- consult your vendor's documentation."

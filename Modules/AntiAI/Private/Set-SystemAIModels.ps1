...
function Set-SystemAIModels {
    ...
    try {
        ...
        # Verify AppPrivacy switch
        $value = Get-ItemProperty -Path $regPath -Name "LetAppsAccessSystemAIModels" -ErrorAction SilentlyContinue
        if ($value.LetAppsAccessSystemAIModels -eq 2) {
            Write-Log -Level DEBUG -Message "Verification SUCCESS: AppPrivacy Master switch is Force Deny" -Module "AntiAI"
        }
        else {
            $result.Errors += "Verification FAILED: AppPrivacy Master switch not set correctly"
        }

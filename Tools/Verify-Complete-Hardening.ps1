...
try {
    ...
    # Layer assignment does NOT reflect detection sophistication -- modern AV/endpoint
    # products in either layer typically employ heuristic + behavioral + reputation + ML.
    ...
    # If third-party endpoint product detected, ASR cannot be enabled by NoID and the
    ...
    if ($securityProduct.Detected) {
        Write-Host "  Third-party endpoint product detected: $($securityProduct.ProductName)" -ForegroundColor Cyan
        Write-Host "  ASR (Attack Surface Reduction) is a Defender-only API and cannot" -ForegroundColor Yellow
        Write-Host "  be verified by NoID when Defender is not the primary engine." -ForegroundColor Yellow
        ...
        # Do NOT count unverified ASR rules as Passed. They are out of scope for verification
        # in this configuration; the AllSettings entry below reports Passed=0 / Failed=0 so
        # the reconciled compliance percentage reflects ACTUAL Defender-side verification.
        $results.AllSettings += [PSCustomObject]@{
            Category      = "ASR"
            ...
            PassedDetails = @([PSCustomObject]@{
                Rule = "ASR rules (Defender-specific) -- not applicable in this configuration"
                Expected = "$EXPECTED_ASR_COUNT ASR rules active on Defender"
                Actual = "$($securityProduct.ProductName) is primary engine; ASR API not available to NoID. Equivalence not verified -- consult $($securityProduct.ProductName) documentation."
            })
            FailedDetails = @()
        }

        Write-Host "  ASR: skipped -- $EXPECTED_ASR_COUNT rules not verifiable (third-party engine)" -ForegroundColor Yellow
    }
    else {
...
try {
    ...
    # NOTE: SRP path rules are verified separately below, since random GUID names are used

<#
.SYNOPSIS
    NoID Privacy Pro - Interactive Menu Interface
...
$Host.UI.RawUI.WindowTitle = "NoID Privacy Pro v2.1.0"
...
function Write-Banner {
    ...
    Write-Host "         NoID PRIVACY PRO v2.1.0          " -ForegroundColor Cyan
    Write-Host "    ========================================" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "    Professional Windows 11 Security Hardening Framework" -ForegroundColor Gray
    Write-Host "    Version 2.1.0 | Build 26200+ | PowerShell 5.1+" -ForegroundColor DarkGray
    Write-Host ""
}
...
function Show-SystemInfo {
    ...
    Write-Host ""
    Write-ColorText "====================================================================" -Color Cyan
    Write-ColorText "  Press ENTER to return to the main menu..." -Color White
    Write-ColorText "====================================================================" -Color Cyan
    Write-Host ""
    [void](Read-Host)
    Write-Host ""
...
function Show-ModuleMenu {
    ...
    $modules = @(
        @{ Key = "1"; Name = "SecurityBaseline"; Description = "Microsoft Security Baseline (425 settings)"; Enabled = $true }
        @{ Key = "2"; Name = "ASR"; Description = "Attack Surface Reduction (19 rules)"; Enabled = $true }
        @{ Key = "3"; Name = "DNS"; Description = "Secure DNS with DoH (Cloudflare/Quad9/AdGuard)"; Enabled = $true }
...
function Invoke-HardeningWorkflow {
    ...
    Write-Host ""
    Write-Step "Initializing NoID Privacy Pro Framework..." -Status INFO
    Write-ColorText "      " -Color Gray -NoNewline
    Write-ColorText "> Automatic backup will be created before changes" -Color DarkGray
    Write-Host ""
    
    try {
        # Determine module parameter
        $moduleParam = if ($SelectedModules.Count -eq 1) {
            $SelectedModules[0]
        }
        else {
            "All"
        ...
        & $frameworkScript -Module $moduleParam -VerboseLogging
        ...
        Write-ColorText "$($SelectedModules.Count)" -Color White
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-ColorText "  Some errors occurred during hardening. Check the log file for details." -Color Yellow
        }
...
try {
    ...
    while ($true) {
        # Hard clear before each main menu redraw to avoid any residual output
        Clear-Host
        Show-MainMenu
        $choice = Read-Host
        ...
        switch ($choice.ToUpper()) {
            "A" {
                ...
                elseif ($moduleChoice -eq "99") {
                    $selectedModules = $modules | Where-Object { $_.Enabled } | ForEach-Object { $_.Name }
                }
                else {
                    $selectedModule = $modules | Where-Object { $_.Key -eq $moduleChoice }
                    if ($selectedModule -and $selectedModule.Enabled) {
                        $selectedModules = @($selectedModule.Name)
                    }
                    else {
                ...
                Invoke-HardeningWorkflow -SelectedModules $selectedModules
            ...
            "X" {
                Write-Host ""
                Write-ColorText "  Thank you for using NoID Privacy Pro!" -Color Cyan
                Write-Host ""
                exit 0
            }
            default {

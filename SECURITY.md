...
### Design Principles
- ✅ **No External Dependencies**: No third-party DLLs or executables; no LGPO.exe
- ⏳ **Code Signing**: Currently unsigned. Code signing is on the roadmap but not yet implemented — users should verify SHA256 checksums (see "Before Running" below) until signing ships
- ✅ **Verification**: 630+ automated compliance checks performed by `Tools/Verify-Complete-Hardening.ps1`
- ✅ **Rollback**: Backup & restore functionality for all applied module settings (see Reversibility notes in README)

...
> **Executive Summary:** 
> The large majority of settings improve security without visible impact for home users. A small number of settings with potential user-facing impact (BitLocker USB enforcement, Password Policy minimum length, Domain-only settings) have been adjusted or documented to remain usable on standalone Home/Pro systems.
...
- **PIN / FaceID:** Unaffected. You can still use Windows Hello PIN (4 digits minimum, up to 127 characters supported since Windows 10 1709) to sign in. The complex password requirement only applies to the underlying account password, not to Windows Hello PIN.

...
NoID Privacy Pro supports fully automated, non-interactive execution for CI/CD pipelines, group policy deployment, and mass system hardening. This guide explains how to run the framework without any interactive prompts.
...
- `"Cloudflare"` (default, fastest)
- `"Quad9"` (Swiss privacy, malware blocking)
...
steps:
  - task: PowerShell@2
    displayName: 'NoID Privacy Pro Hardening'
    inputs:
      targetType: 'filePath'
...
1. Copy NoID Privacy Pro to network share:
   ```
...
$env:NOIDPRIVACY_DNS_PROVIDER = "Cloudflare"
...
<#
.SYNOPSIS
    Enterprise deployment wrapper for NoID Privacy Pro
...
try {
    ...
    # Run hardening
    Write-Output "Starting NoID Privacy Pro hardening..."
    ...
    # Send report email
    $emailBody = @"
NoID Privacy Pro Hardening Report

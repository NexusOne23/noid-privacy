...
## Environment Variables (Roadmap — Not Yet Implemented)

Environment-variable based configuration (e.g. `NOIDPRIVACY_DNS_PROVIDER`,
`NOIDPRIVACY_PRIVACY_MODE`, `NOIDPRIVACY_NONINTERACTIVE`) is on the roadmap
but **not yet implemented**. Until it ships, please use `config.json` (see
sections above) or the `-Module` command-line parameter.

If env-var support is important for your CI/CD pipeline, please file a feature
request on GitHub Discussions.

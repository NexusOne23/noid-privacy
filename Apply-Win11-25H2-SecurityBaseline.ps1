...
$moduleDependencies = @{
    ...
    'RegistryOwnership' = @('Common', 'Localization') # TrustedInstaller Registry Management
    'TaskOwnership' = @('Common', 'Localization')     # v1.8.3: TrustedInstaller Task Management
    ...
    'Telemetry' = @('Common', 'Localization', 'TaskOwnership')    # v1.8.3: Needs TaskOwnership        # Braucht Helper-Functions + Strings
    'Performance' = @('Common', 'Localization', 'TaskOwnership')  # v1.8.3: Needs TaskOwnership
...
$modulePriority = @{
    ...
    'RegistryOwnership' = 3 # Registry Ownership Management (vor Core!)
    'TaskOwnership' = 4     # v1.8.3: Must load early (used by Performance/Telemetry)
    'WindowsUpdate' = 5     # Windows Update Defaults (vor Core, da Core es braucht)
    'Core' = 6              # KRITISCH - System-Validierung (braucht RegistryOwnership!)
    'ASR' = 7               # Attack Surface Reduction
    'Advanced' = 8          # VBS, BitLocker, LAPS
    'DNS-Common' = 9        # DNS Common helpers (MUST load before DNS-Providers!)
    'DNS-Providers' = 10    # Multi-Provider DNS (Cloudflare, AdGuard, NextDNS, Quad9)
    'DNS' = 11              # DNS Security (depends on DNS-Common + DNS-Providers!)
    'Bloatware' = 12        # App-Removal
    'Telemetry' = 13        # Privacy
    'Performance' = 14      # Optimierung
    'UAC' = 15              # UAC Settings
    'AI' = 16               # AI Lockdown (KRITISCH fuer Privacy!)
    'WirelessDisplay' = 17  # Wireless Display / Miracast
    'OneDrive' = 18         # OneDrive Privacy Hardening
    'Edge' = 19             # Microsoft Edge Security Baseline
    'Interactive' = 20      # Menue (braucht alle anderen)
}
    ...
    if (Test-Path $modulePath) {
        try {
            ...
            switch ($moduleName) {
                ...
                'TaskOwnership' { $moduleLoaded = $null -ne (Get-Command 'Disable-ScheduledTaskSmart' -ErrorAction SilentlyContinue) }
...
Write-Host "=============================================================================" -ForegroundColor Cyan
Write-Host ""
Write-Host "  Version: 1.8.3 | ASR Mode: $Mode" -ForegroundColor Cyan
if ($Interactive) {

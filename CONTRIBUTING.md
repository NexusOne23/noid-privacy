...
> **IMPORTANT:** Replace ALL placeholders (`<...>`) below before committing. A CI check
> rejects PRs that still contain `<your-github-username>` or `<NEW-GUID-HERE>`.
> Generate a fresh GUID with `[guid]::NewGuid()` — do NOT copy any GUID from the
> "Real Example" section verbatim.

```powershell
@{
    RootModule = 'YourModule.psm1'
    ModuleVersion = '2.2.4'
    GUID = '<NEW-GUID-HERE>'  # Generate with [guid]::NewGuid()
    Author = '<your-name>'
    ...
    PrivateData = @{
        PSData = @{
            ...
            ProjectUri = 'https://github.com/<your-github-username>/<your-fork-name>'
            ReleaseNotes = @"
...
**Real example: `Modules/AdvancedSecurity/AdvancedSecurity.psd1`.** Open that file
directly to see the live manifest with the actual GUID and metadata. Do not
copy GUIDs across modules — each module needs its own unique GUID.
...
```powershell
# 1) Prepare the test environment (once)
.\Tests\Setup-TestEnvironment.ps1

# 2) Run all tests (Unit + Integration + Validation)
.\Tests\Run-Tests.ps1

# 3) Run only a specific test type
...
# Alternative: simpler runner for all tests
